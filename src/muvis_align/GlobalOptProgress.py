...
The optimiser runs that inner loop more than once - the outer loop drops the worst edge and runs
it again, as often as it takes - and how many times is not knowable in advance. Each pass
therefore opens a phase of its own, taking a slice of whatever is left of the bar, which is what
NapariPhaseProgress already does for an operation that runs more phases than it declared: the bar
keeps moving across every pass without any one of them filling it.
...
class GlobalOptProgress:
    ...
    def __init__(self, progress_factory=None, desc='Global registration',
                 max_iter=None, heartbeat_seconds=None):
    ...
    def note_iteration(self, iteration, max_residual=None):
        """One pass of the inner optimisation loop finished."""
        try:
            iteration = int(iteration)
        except (TypeError, ValueError):
            return
        if self._iteration is not None and iteration <= self._iteration:
            # the count restarting means the outer loop dropped an edge and began the inner loop
            # again - a new phase, out of what is left of the bar, not a jump backwards in this one
            self._end_pass(None)
            self._start_pass()
        done_so_far = self._iteration + 1 if self._iteration is not None else 0
        steps = iteration + 1 - done_so_far
        self._iteration = iteration
        self._saw_iterations = True
        self._open_phase()
        if self._phase is not None and steps > 0:
            self._phase.update(steps)
    ...
    def _open_phase(self):
        """Opened at the first iteration of a pass, not at the start of one: the ceiling the pass
        counts against is the max_iter upstream logs on its way in, which it writes after this is
        already listening. A phase opened any earlier would size itself against the default and
        report a 100-iteration pass as a fifth done."""
        if self._phase is not None or self.progress_factory is None:
            return
        self._phase = self.progress_factory(total=self.max_iter, desc=self.desc)
        self._phase.__enter__()

    def _end_pass(self, exc_type):
    ...
    def _log_heartbeat(self, max_residual):
        ...
        if max_residual is not None:
            try:
                ...
                pass
        logging.info(f'{self.desc}: optimisation pass {self._pass},'
